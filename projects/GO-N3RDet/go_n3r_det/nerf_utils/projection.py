...
class Projector():
    ...
    def compute(self,
        ...
        train_imgs = train_imgs.permute(0, 3, 1, 2).contiguous()
        ...
        pixel_locations, mask_in_front = self.compute_projections(
            xyz, train_cameras)
        normalized_pixel_locations = self.normalize(pixel_locations, h, w).contiguous()
        # rgb sampling
        ...
        if featmaps is not None:
            if grid_sample:
                feat_sampled = F.grid_sample(
                    featmaps.contiguous(), normalized_pixel_locations.contiguous(), align_corners=True)
